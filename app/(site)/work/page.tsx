...
function ProjectSection({ project, index }: { project: Project; index: number }) {
  ...
  return (
    <article className="grid overflow-hidden border border-border bg-card lg:grid-cols-2">
      <Link href={`/work/${project.slug}`} className={`group relative block overflow-hidden bg-muted lg:h-full ${reversed ? 'lg:order-2' : ''}`}>
        <PortfolioCoverImage src={projectImage(project)} alt={project.cover_image_alt || project.title} sizes="(min-width: 1024px) 50vw, 100vw" fillContainer className="transition-transform duration-500 group-hover:scale-[1.02]" />
